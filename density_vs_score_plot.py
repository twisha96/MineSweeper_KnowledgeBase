...
y_inference = []
legend = []
density_array = [0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0]
...
for dim in dim_array:
    ...
    for density in density_array:
        ...
        for k in range(iterations):
            ...
            score_inf, random_picks_inf, exec_time_inf = inf.start_baseline(board2)
    ...
    axs[0].plot(density_array, y_baseline)
    axs[1].plot(density_array, y_inference)
    legend.append(dim)

axs[0].title.set_text('Baseline Algorithm')
axs[1].title.set_text('Inference Algorithm')

plt.legend(legend, title='Board Dimension')
plt.show()
